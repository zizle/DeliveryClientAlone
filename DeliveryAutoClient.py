...
class CheckUpdatingVersion(QThread):
    ...
    def run(self):
        ...
        try:
            r = request_get(
                url=HTTP_SERVER + 'update_delivery/?version=' + str(version),
...
class DownloadNewVersion(QThread):
    ...
    def download_file(self, file_name):
        ...
        r = request_get(
            url=HTTP_SERVER + 'downloading_delivery/',
            headers={'User-Agent': 'RuiDa_DeliveryClient', 'Content-Type': 'application/json;charset=utf8'},
...
class StartPage(QLabel):
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle('交割信息系统自动更新程序')
        self._pressed = False
        self._mouse_pos = None
        icon_path = os.path.join(BASE_DIR, "media/logo.png")
        pix_path = os.path.join(BASE_DIR, "media/updating.png")
        ...
        self.setFont(font)
        self.show_text = QLabel("欢迎使用交割信息系统\n正在检查新版本...", self)
    ...
    def version_checked(self, result):
        # print('版本检测成功', result)
        if result['update']:
            self.show_text.setText("欢迎使用交割信息系统,检测到新版本{}\n正在准备更新...".format(result['version']))
            # 线程下载文件到目录中
            ...
            self.show_text.setText("欢迎使用交割信息系统\n当前已是最新版本!")
    ...
    def update_finished(self):
        # 更新完成，执行系统主程序Delivery.py
        if os.path.exists("Delivery.exe"):
            Popen('Delivery.exe', shell=False)
        self.close()
        sys.exit()
