...
class DeliveryPage(QScrollArea):
    def __init__(self, *args, **kwargs):
        ...
        self.logout_button = QPushButton("注销", self)
        ...
        self.logout_button.clicked.connect(self.user_to_logout)
        self.register_button.clicked.connect(self.user_to_register)
        ...
        self.username_label.setObjectName('userLabel')
        ...
        #menuBar{background-color:rgb(34,102,175)}
        #loginBtn,#logoutBtn,#registerBtn,#userLabel{border:none;padding:0 4px;background-color:rgb(34,102,175);min-height:22px}
        #logoutBtn:hover{color:rgb(180,100,120)}
        #loginBtn:hover,#registerBtn:hover{color:rgb(100,200,220)}
        """)
        self.container.setStyleSheet("""
    ...
    def user_to_login(self):
        ...
        def login_successfully(user_data):
            ...
            # 写入token
            app_dawn.setValue('AUTHORIZATION', user_data['utoken'])
        popup = LoginPopup(self)
        popup.user_listed.connect(login_successfully)
        popup.exec_()

    def user_to_logout(self):
        from settings import app_dawn
        app_dawn.remove('AUTHORIZATION')
